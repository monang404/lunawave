...
def count_lines(root: Path, ext: str) -> int:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fn in filenames:
            if fn.endswith(ext):
                try:
                    total += sum(1 for _ in (Path(dirpath) / fn).open(encoding="utf-8", errors="replace"))
                except Exception:
                    pass
    return total
...
def build_generated_block(root: Path) -> str:
    ...
    lines = [
        f"> **Auto-generated:** {date.today().isoformat()} oleh `scripts/generate_report.py`  \n"
