...
def check_big_files(project_root: Path) -> CheckResult:
    ...
    if critical:
        return CheckResult(
            "Big Files", "FAIL",
            f"{len(critical)} file kritis (>{CRITICAL_FILE_THRESHOLD} baris) perlu dipecah",
            items,
        )
    return CheckResult(
        "Big Files", "WARN",
        f"{len(big)} file antara {BIG_FILE_THRESHOLD}-{CRITICAL_FILE_THRESHOLD} baris — perhatikan",
        items,
    )
...
def check_pending_items(project_root: Path) -> CheckResult:
    ...
    if issues:
        return CheckResult(
            "Pending Items", "WARN",
            f"{len(issues)} item pending", issues,
        )
