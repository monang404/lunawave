...
class ServerManager(tk.Tk):
    ...
    def _on_start(self):
        ...
        self.server_process = process.ServerProcess(str(BASE_DIR), port, on_log=self._write_log)
        try:
            self.server_process.start()
            self._write_log(f"Server process created — PID {self.server_process.process.pid}", "ok")
    ...
    def _wait_stop(self):
        try:
            ...
            self._write_log(f"Force killed: {e}", "err")
