...
class UIBuilder:
    def __init__(self, bg, bg_surface, bg_card, accent, text_1, text_2, text_3, green, red, border, base_dir):
        self.BG = bg
    ...
    def build_ui(self, app):
        ...
        tk.Label(
            ...
            bg=self.BG_SURFACE, fg=self.ACCENT,
            font=("Segoe UI", 18, "bold"),
        ).pack()
        tk.Label(
            header, text="Server Manager",
            bg=self.BG_SURFACE, fg=self.TEXT_3,
            font=("Segoe UI", 9),
        ).pack()
        ...
        app._status_label = tk.Label(
            left, text="Checking...",
            bg=self.BG_CARD, fg=self.TEXT_2,
        ...
        tk.Label(
            ...
            bg=self.BG_CARD, fg=self.TEXT_3,
            font=("Segoe UI", 9),
        ).pack(side="left", padx=(0, 4))

        app._port_entry = tk.Entry(
            port_frame, textvariable=app._port_var,
            bg=self.BG_SURFACE, fg=self.TEXT_1, font=("Consolas", 10),
            width=6, relief="flat", insertbackground=self.TEXT_1,
            justify="center", highlightthickness=1, highlightbackground=self.BORDER
        )
        ...
        app._btn_kill_conflict = tk.Button(
            ...
            fg=self.RED, bg="#2A0A0A", font=("Segoe UI", 8, "bold"),
        ...
        app._btn_start = self._make_btn(app,
            btn_frame, "▶  Start", self.ACCENT, "#2A1F06",
            app._on_start, col=0
        )
        app._btn_stop = self._make_btn(app,
            btn_frame, "■  Stop", self.RED, "#2A0A0A",
            app._on_stop, col=1
        )
        app._btn_restart = self._make_btn(app,
            btn_frame, "↺  Restart", self.TEXT_2, self.BG_CARD,
            app._on_restart, col=2
        )
        app._btn_open = self._make_btn(app,
            btn_frame, "⬡  Open Portal", self.TEXT_2, self.BG_CARD,
            app._on_open, col=3
        )
        ...
        tk.Label(
            ...
            bg=self.BG_CARD, fg=self.TEXT_3, font=("Segoe UI", 8, "bold")
        ).pack(side="left", anchor="w")

        tk.Label(
            admin_frame, text="User: admin  ·  Pass: [Hashed]",
            bg=self.BG_CARD, fg=self.TEXT_2, font=("Segoe UI", 9)
        ).pack(side="left", padx=15)

        btn_reset = tk.Button(
            admin_frame, text="Reset Password",
            bg=self.BG_SURFACE, fg=self.ACCENT, font=("Segoe UI", 8, "bold"),
            relief="flat", cursor="hand2", bd=0,
            activebackground=self.BG_CARD, activeforeground=self.TEXT_1,
            padx=10, command=lambda: on_reset_password(self, self.BASE_DIR, self.BG, self.BG_CARD, self.BG_SURFACE, self.ACCENT, self.TEXT_1, self.TEXT_2, self.TEXT_3, self.RED, self.GREEN, self.BORDER)
        )
        btn_reset.pack(side="right")

        # Hover effect for reset button
        def on_enter_reset(e): btn_reset.config(bg=self.BG, fg=self.TEXT_1)
        def on_leave_reset(e): btn_reset.config(bg=self.BG_SURFACE, fg=self.ACCENT)
        ...
        tk.Button(
            ...
            font=("Segoe UI", 8),
            relief="flat", cursor="hand2", bd=0,
            activebackground=self.BG, activeforeground=self.TEXT_2,
            command=app._clear_log
        ).pack(side="right")

        app._log = scrolledtext.ScrolledText(
            self,
            bg=self.BG_SURFACE, fg=self.TEXT_2,
            font=("Consolas", 8),
            relief="flat", bd=0,
            wrap="word",
