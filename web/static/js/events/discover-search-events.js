...
function initDiscoverSearchEvents() {
    ...
    function resolveDecade(year) {
        if (typeof window.getDecade === "function") return window.getDecade(year);
        if (!year) return null;
    ...
    function buildDecadeOptions() {
        ...
        const allArtists = [].concat(
            (window.store && store.discover_for_you) || [],
            (window.store && store.discover_genre_affinity_artists) || [],
            (window.store && store.discover_unheard) || []
        );
    ...
    function sendSearch(query) {
        if (typeof enterDiscoverSearchLoading === "function") enterDiscoverSearchLoading(query);
    ...
    if (clearBtn) {
        clearBtn.addEventListener("click", () => {
            ...
            if (typeof exitDiscoverSearchMode === "function") exitDiscoverSearchMode();
            input.focus();
        });
    }
