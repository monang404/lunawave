"""
Module: tests.unit.automation.test_repo_index
...
_AUTOMATION_DIR = str(Path(__file__).resolve().parents[3] / "automation")
if _AUTOMATION_DIR not in sys.path:
    # automation/shared/repo_index.py mengimpor `shared.skip_dirs` (bukan
    # `automation.shared.skip_dirs`) karena tiap script CLI di automation/
    # menambahkan automation/ ke sys.path sendiri saat dijalankan langsung.
    # Test butuh bootstrap yang sama karena import di sini tidak lewat CLI.
    sys.path.insert(0, _AUTOMATION_DIR)


@pytest.fixture()
def repo_index(monkeypatch, tmp_path):
    import automation.shared.repo_index as ri
