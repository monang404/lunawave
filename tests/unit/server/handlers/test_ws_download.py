...
async def test_handle_download_command_delete_download(mock_exists, mock_remove, mock_discover_service):
    mock_db = AsyncMock()
    mock_track = MagicMock()
    ...
    with patch("server.handlers.ws_download.dict_to_track", return_value=mock_track):
        await handle_download_command("delete_download", {"video_id": "test_vid"}, mock_db, mock_manager, mock_state)
    ...
    mock_manager.broadcast.assert_any_call({"type": "log", "data": f"Unduhan dihapus: {mock_track.title}"})
