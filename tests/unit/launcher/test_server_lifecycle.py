...
def test_module_importable_without_tkinter():
    ...
    # import launcher.server_lifecycle in it. A shared-process check via
    ...
    script = (
        ...
        "import launcher.server_lifecycle\n"
    )
...
def _lifecycle(monkeypatch):
    monkeypatch.setattr("launcher.server_lifecycle.threading.Thread", _ImmediateThread)
    logs = []
    lc = ServerLifecycle(
        "/fake/base",
...
def test_start_noop_when_already_running(monkeypatch):
    ...
    with patch("launcher.server_lifecycle.process.ServerProcess") as mock_sp:
        lc.start(8765)
...
def test_start_creates_process_when_port_free(monkeypatch):
    ...
    monkeypatch.setattr("launcher.server_lifecycle.network.check_port_in_use", lambda port: False)

    mock_proc = MagicMock()
    mock_proc.process.pid = 4242
    mock_proc.is_running.return_value = False  # ends wait_for_ready's loop immediately

    with patch("launcher.server_lifecycle.process.ServerProcess", return_value=mock_proc) as mock_sp:
        with patch("launcher.server_lifecycle.process.kill_mpv") as mock_kill_mpv:
            lc.start(8765)

    mock_kill_mpv.assert_called_once()
    mock_sp.assert_called_once_with("/fake/base", 8765, on_log=lc.on_log)
...
def test_start_kills_conflicting_process_then_starts(monkeypatch):
    ...
    monkeypatch.setattr(
        "launcher.server_lifecycle.network.check_port_in_use", lambda port: next(port_checks)
    )
    monkeypatch.setattr(
        "launcher.server_lifecycle.network.get_pid_occupying_port", lambda port: 999
    )
    monkeypatch.setattr("launcher.server_lifecycle.time.sleep", lambda s: None)

    mock_proc = MagicMock()
    mock_proc.process.pid = 1
    mock_proc.is_running.return_value = False

    with patch("launcher.server_lifecycle.process.ServerProcess", return_value=mock_proc):
        with patch("launcher.server_lifecycle.process.kill_process_tree") as mock_kpt:
            with patch("launcher.server_lifecycle.process.kill_mpv"):
                lc.start(8765)
...
def test_start_gives_up_when_port_stays_in_use(monkeypatch):
    lc, logs = _lifecycle(monkeypatch)
    monkeypatch.setattr("launcher.server_lifecycle.network.check_port_in_use", lambda port: True)
    monkeypatch.setattr(
        "launcher.server_lifecycle.network.get_pid_occupying_port", lambda port: 999
    )
    monkeypatch.setattr("launcher.server_lifecycle.time.sleep", lambda s: None)

    with patch("launcher.server_lifecycle.process.kill_process_tree"):
        with patch("launcher.server_lifecycle.process.kill_mpv"):
            with patch("launcher.server_lifecycle.process.ServerProcess") as mock_sp:
                lc.start(8765)
...
def test_stop_noop_when_not_running(monkeypatch):
    lc, _ = _lifecycle(monkeypatch)
    with patch("launcher.server_lifecycle.threading.Thread") as mock_thread:
        lc.stop()
...
def test_kill_conflict_success(monkeypatch):
    lc, logs = _lifecycle(monkeypatch)
    monkeypatch.setattr(
        "launcher.server_lifecycle.network.get_pid_occupying_port", lambda port: 555
    )
    monkeypatch.setattr("launcher.server_lifecycle.network.check_port_in_use", lambda port: False)
    monkeypatch.setattr("launcher.server_lifecycle.time.sleep", lambda s: None)
...
def test_kill_conflict_no_pid_found(monkeypatch):
    lc, logs = _lifecycle(monkeypatch)
    monkeypatch.setattr(
        "launcher.server_lifecycle.network.get_pid_occupying_port", lambda port: None
    )
...
def test_run_dependency_check_reports_result(monkeypatch):
    ...
    with patch("launcher.dep_checker.DependencyChecker", return_value=mock_checker):
