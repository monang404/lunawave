...
def test_kill_process_tree_win32_failure_is_fail_safe_and_logged(monkeypatch):
    ...
    with patch("launcher.process.sys.platform", "win32"):
        with patch("launcher.process.subprocess.run", side_effect=OSError("taskkill missing")):
...
def test_kill_mpv_win32_failure_is_fail_safe_and_logged(monkeypatch):
    calls = _capture_debug(monkeypatch)
    with patch("launcher.process.sys.platform", "win32"):
        with patch("launcher.process.subprocess.run", side_effect=OSError("taskkill missing")):
            kill_mpv()  # must not raise
    assert [event for event, _ in calls] == ["kill_mpv_failed"]


def test_kill_mpv_unix_failure_is_fail_safe_and_logged(monkeypatch):
    calls = _capture_debug(monkeypatch)
    with patch("launcher.process.sys.platform", "linux"):
        with patch("launcher.process.subprocess.run", side_effect=OSError("pkill missing")):
            kill_mpv()  # must not raise
    assert [event for event, _ in calls] == ["kill_mpv_failed"]
...
def test_stop_force_kill_failure_is_fail_safe_and_logged(monkeypatch):
    calls = _capture_debug(monkeypatch)
    with patch("launcher.process.kill_process_tree"):
        sp = ServerProcess("/fake/cwd", 8080)
        mock_proc = MagicMock()
        mock_proc.poll.return_value = None  # is_running -> True
        mock_proc.pid = 999
