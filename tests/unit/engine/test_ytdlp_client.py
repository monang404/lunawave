...
class TestPickAudioUrl:
    def test_returns_audio_only_format_url(self):
        ...
        assert client._pick_audio_url(info) == "https://audio.url"
    ...
    def test_falls_back_to_top_level_url_when_no_audio_only_format(self):
        ...
        assert client._pick_audio_url(info) == "https://fallback.url"
    ...
    def test_prefers_last_audio_only_format_reversed(self):
        ...
        assert client._pick_audio_url(info) == "https://audio2.url"
...
class TestSearch:
    async def test_returns_tracks_for_valid_entries(self):
        ...
        with patch.object(client, "_extract_sync", return_value=raw):
    ...
    async def test_returns_empty_list_when_no_entries(self):
        client = YtDlpClient()
        with patch.object(client, "_extract_sync", return_value={"entries": []}):
...
class TestGetStreamUrl:
    async def test_returns_audio_url_on_success(self):
        ...
        with patch.object(client, "_extract_sync", return_value=info):
    ...
    async def test_raises_runtime_error_when_extract_returns_none(self):
        ...
        with patch.object(client, "_extract_sync", return_value=None):
    ...
    async def test_wraps_arbitrary_exception_as_runtime_error(self):
        ...
        with patch.object(client, "_extract_sync", side_effect=ConnectionError("net down")):
...
class TestDownloadMp3:
    async def test_returns_expected_mp3_path(self, tmp_path, monkeypatch):
        ...
        with patch.object(client, "_download_sync", return_value=None):
            monkeypatch.setattr("engine.ytdlp_client.CACHE_DIR", tmp_path)
