...
def ytdlp_client():
    client = YtDlpClient()
    # Mock executor to just call synchronously for tests
    client._executor = MagicMock()
    # We will mock the asyncio run_in_executor to not actually run in executor
    return client
...
async def test_ytdlp_search(ytdlp_client):
    with patch.object(asyncio.get_running_loop(), "run_in_executor", new_callable=AsyncMock) as mock_run:
        mock_run.return_value = {
            "entries": [
                {"id": "vid1", "title": "Test 1", "duration": 180, "uploader": "Artist 1"},
...
async def test_ytdlp_get_stream_url_success(ytdlp_client):
    with patch.object(asyncio.get_running_loop(), "run_in_executor", new_callable=AsyncMock) as mock_run:
        ...
        url = await ytdlp_client.get_stream_url("vid1")
    ...
    with patch.object(asyncio.get_running_loop(), "run_in_executor", new_callable=AsyncMock) as mock_run:
        with patch.object(asyncio, "wait_for") as mock_wait:
            ...
            with pytest.raises(RuntimeError) as exc_info:
                await ytdlp_client.get_stream_url("vid1")
            assert "Timeout" in str(exc_info.value)

@pytest.mark.asyncio
async def test_ytdlp_download_mp3(ytdlp_client):
    with patch.object(asyncio.get_running_loop(), "run_in_executor", new_callable=AsyncMock) as mock_run:
        path = await ytdlp_client.download_mp3("vid1")
        assert "vid1.mp3" in path
